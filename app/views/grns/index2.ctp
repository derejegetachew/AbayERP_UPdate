var store_parent_grns = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'grns', 'action' => 'list_data', $parent_id)); ?>'	})
});


function AddParentGrn() {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'grns', 'action' => 'add', $parent_id)); ?>',
...
function EditParentGrn(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'grns', 'action' => 'edit')); ?>/'+id+'/<?php echo $parent_id; ?>',
		success: function(response, opts) {
			var parent_grn_data = response.responseText;
