...
,			['CompetenceLevel.name', '<?php __('CompetenceLevel'); ?>']
,			['CompetenceDefinition.definition', '<?php __('Definition'); ?>']
		]
	}),
	mode: 'local',
...
var competenceDefinitionSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					competenceDefinitionSearchWindow.close();
...
var competenceDefinitionSearchWindow = new Ext.Window({
	title: '<?php __('Search CompetenceDefinition'); ?>',
	modal: true,
	items: competenceDefinitionSearchForm,
	resizable: false,
	width: 550,
	height: 400,
