...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['BpItem.name', '<?php __('BpItem'); ?>']
,			['BpMonth.name', '<?php __('BpMonth'); ?>']
,			['BudgetYear.name', '<?php __('BudgetYear'); ?>']
,			['BpCumulative.plan', '<?php __('Plan'); ?>']
,			['BpCumulative.actual', '<?php __('Actual'); ?>']
,			['BpCumulative.cumilativePlan', '<?php __('CumilativePlan'); ?>']
,			['BpCumulative.cumilativeActual', '<?php __('CumilativeActual'); ?>']
		]
	}),
	mode: 'local',
...
var bpCumulativeSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					bpCumulativeSearchWindow.close();
...
var bpCumulativeSearchWindow = new Ext.Window({
	title: '<?php __('Search BpCumulative'); ?>',
	modal: true,
	items: bpCumulativeSearchForm,
	resizable: false,
	width: 550,
	height: 400,
