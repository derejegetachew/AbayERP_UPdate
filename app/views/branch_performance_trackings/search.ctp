...
,			['BranchPerformanceTracking.goal', '<?php __('Goal'); ?>']
,			['BranchPerformanceTracking.date', '<?php __('Date'); ?>']
,			['BranchPerformanceTracking.value', '<?php __('Value'); ?>']
		]
	}),
	mode: 'local',
...
var branchPerformanceTrackingSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					branchPerformanceTrackingSearchWindow.close();
...
var branchPerformanceTrackingSearchWindow = new Ext.Window({
	title: '<?php __('Search BranchPerformanceTracking'); ?>',
	modal: true,
	items: branchPerformanceTrackingSearchForm,
	resizable: false,
	width: 550,
	height: 400,
