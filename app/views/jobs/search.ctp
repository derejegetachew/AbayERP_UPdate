...
function AddTextBoxValues(){
  ...
  if(Ext.getCmp('Name').getValue()!=''){
  	AddParameter("LIKE","Job.name",Ext.getCmp('Name').getValue());
...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Job.name', '<?php __('Name'); ?>']
,			['Job.description', '<?php __('Description'); ?>']
,			['Job.start_date', '<?php __('Start Date'); ?>']
,			['Job.end_date', '<?php __('End Date'); ?>']
...
var jobSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
...
var jobSearchWindow = new Ext.Window({
	title: '<?php __('Search Job'); ?>',
	modal: true,
	items: jobSearchForm,
	resizable: false,
	width: 550,
