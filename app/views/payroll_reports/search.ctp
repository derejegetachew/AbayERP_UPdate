...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['PayrollReport.date', '<?php __('Date'); ?>']
...
var payrollReportSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					payrollReportSearchWindow.close();
...
var payrollReportSearchWindow = new Ext.Window({
	title: '<?php __('Search PayrollReport'); ?>',
	modal: true,
	items: payrollReportSearchForm,
	resizable: false,
	width: 550,
	height: 400,
