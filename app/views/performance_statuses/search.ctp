...
,			['PerformanceStatus.quarter', '<?php __('Quarter'); ?>']
,			['PerformanceStatus.status', '<?php __('Status'); ?>']
		]
	}),
	mode: 'local',
...
var performanceStatusSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					performanceStatusSearchWindow.close();
...
var performanceStatusSearchWindow = new Ext.Window({
	title: '<?php __('Search PerformanceStatus'); ?>',
	modal: true,
	items: performanceStatusSearchForm,
	resizable: false,
	width: 550,
	height: 400,
