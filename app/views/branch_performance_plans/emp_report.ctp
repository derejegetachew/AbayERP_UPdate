...
function my_technical_report(emp, budget_year, quarter){
	...
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'branchPerformancePlans', 'action' => 'br_emp_technical_report')); ?>/'+data,
...
function my_tracking_report(emp, budget_year, quarter){
	...
	var data = emp+'-'+budget_year+'-'+quarter;
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'branchPerformancePlans', 'action' => 'br_emp_tracking_report')); ?>/'+data,
		success: function(response, opts) {
			var competenceCategories_data = response.responseText;
			...
			items: [
				...
				
                {
					...
					store: new Ext.data.ArrayStore({
						...
						data: [						
						['technical','Technical'],['tracking', 'Tracking'],						
						]
