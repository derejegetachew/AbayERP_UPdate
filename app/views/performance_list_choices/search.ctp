...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['PerformanceListChoice.name', '<?php __('Name'); ?>']
...
var performanceListChoiceSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					performanceListChoiceSearchWindow.close();
...
var performanceListChoiceSearchWindow = new Ext.Window({
	title: '<?php __('Search PerformanceListChoice'); ?>',
	modal: true,
	items: performanceListChoiceSearchForm,
	resizable: false,
	width: 550,
	height: 400,
