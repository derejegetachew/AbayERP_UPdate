//<script>
	...
	reader: new Ext.data.JsonReader({
            ...
            fields: [
                ...
                'experience', 'education', 'children','language','EmployeeDetail','DisciplinaryRecord', 'created'		
                ]
	}),
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'employees', 'action' => 'list_data')); ?>'
	}),	
	...
	function disciplinary_records(id) {
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'disciplinary_records', 'action' => 'index2')); ?>/'+id,
            success: function(response, opts) {
                var parent_offsprings_data = response.responseText;

                eval(parent_offsprings_data);

                parentDisciplinaryRecordsViewWindow.show();
            },
            failure: function(response, opts) {
                Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the campus view form. Error code'); ?>: ' + response.status);
            }
...
function subordinates(id){
   window.open("<?php echo $this->Html->url(array('controller' => 'Supervisors', 'action' => 'subreport')); ?>/"+id);
    }	
            ...
            items: [
                {
                    ...
                    text: 'Show Subordinates',
                    handler: function() {
                        subordinates(record.get('id'));
                    }
                },{
                    ...
                    text: 'Disciplinary Records (' + record.get('DisciplinaryRecord') + ')',
                    handler: function() {
                        disciplinary_records(record.get('id'));
                    }
                }
                ,{
                    text: 'Print Experiance',
                    handler: function() {
                       		window.open("http://10.1.85.85:8080/iportal/frameset?__report=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs%5Cemp_experiance.rptdesign&__format=html&__svg=true&__locale=en_US&__timezone=EAT&__masterpage=true&__rtl=false&__cubememsize=1000&__resourceFolder=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs&1302871916&user_id="+record.get('user_id'), "_blank");
                    }
                },{
                  text: 'Print Profile(For Credit)',
                    handler: function() {
                       		window.open("http://10.1.85.85:8080/iportal/frameset?__report=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs%5Cemp_profile.rptdesign&__format=html&__svg=true&__locale=en_US&__timezone=EAT&__masterpage=true&__rtl=false&__cubememsize=1000&__resourceFolder=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs&465082654&user_id="+record.get('user_id'), "_blank");
                    }
