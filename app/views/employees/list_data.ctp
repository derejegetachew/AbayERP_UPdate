...
?>
{
	...
	rows: [
<?php $st = false; foreach($employees as $employee){ if(!empty($employee['User']) && !empty($employee['User']['Person'])){ if($st) echo ","; $pre = ""; $post = '';  ?>			{
            "id":"<?php echo $employee['Employee']['id']; ?>",
             "user_id":"<?php echo $employee['Employee']['user_id']; ?>",
			...
			"DisciplinaryRecord":"<?php echo count($employee['DisciplinaryRecord']); ?>",
