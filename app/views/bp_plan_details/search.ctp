...
,			['BpPlan.name', '<?php __('BpPlan'); ?>']
,			['BpPlanDetail.amount', '<?php __('Amount'); ?>']
		]
	}),
	mode: 'local',
...
var bpPlanDetailSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					bpPlanDetailSearchWindow.close();
...
var bpPlanDetailSearchWindow = new Ext.Window({
	title: '<?php __('Search BpPlanDetail'); ?>',
	modal: true,
	items: bpPlanDetailSearchForm,
	resizable: false,
	width: 550,
	height: 400,
