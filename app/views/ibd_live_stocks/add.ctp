		...
		?>
		var IbdLiveStockAddForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 150,
			labelAlign: 'right',
            ...
            items:[
            {
				...
				?>,
				{
					...
					anchor:'100%',
					name :'data[IbdLiveStock][quantity_mt]',
			     ...
			     },
			     {
					...
					fieldLabel:'Price',
					anchor:'100%',
					name :'data[IbdLiveStock][price_mt]',
					enableKeyEvents:true,
					listeners : {
						 keyup: function(field,e){
							 var value=e.target.value;
				...
				,
				<?php 
					$options = array('id'=>'total_price','readOnly'=>true);
				...
				?>]},
				  {
			         ...
			         columnWidth:.5,
						...
						items: [
				<?php 
					$options = array('xtype'=>'combo','valueField'=>'name');
					if(isset($parent_id))
				...
				<?php 
					$options = array('allowBlank'=>false,'allowBlank'=>false);
				...
				?>,
				<?php 
					$options = array('xtype'=>'combo','valueField'=>'name','allowBlank'=>false);
					if(isset($parent_id))
						$options['hidden'] = $parent_id;
					else
		...
		var IbdLiveStockAddWindow = new Ext.Window({
			title: '<?php __('Add Ibd Live Stock'); ?>',
			width: 700,
			minWidth: 400,
			autoHeight: true,
			layout: 'fit',
			...
			buttonAlign:'right',
			items: IbdLiveStockAddForm,
			tools: [{
				id: 'refresh',
				qtip: 'Reset',
				handler: function () {
					IbdLiveStockAddForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					Ext.Msg.show({
						...
						msg: 'This form is used to insert a new Ibd Live Stock.',
				...
				handler: function () {
					if(IbdLiveStockAddWindow.collapsed)
						IbdLiveStockAddWindow.expand(true);
					else
						IbdLiveStockAddWindow.collapse(true);
				}
			}],
			buttons: [  {
				text: '<?php __('Save'); ?>',
				handler: function(btn){
					IbdLiveStockAddForm.getForm().submit({
