//<script>
	...
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'purchaseOrderItems', 'action' => 'list_data')); ?>'
	}),	
        sortInfo:{field: 'purchase_order_id', direction: "ASC"},
	groupField: 'item_id'
    });
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'purchaseOrderItems', 'action' => 'edit')); ?>/'+id,
            success: function(response, opts) {
                var purchaseOrderItem_data = response.responseText;
                            ...
                            data : [
                                ...
                                <?php $st = false;
                                    foreach ($purchaseorders as $item) {
                                        if ($st) echo ",
							"; ?>['<?php echo $item['PurchaseOrder']['id']; ?>' ,'<?php echo $item['PurchaseOrder']['name']; ?>']<?php $st = true;
                                    } ?>
                            ]
                        }),
                        displayField : 'name',
