...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Takentraining.name', '<?php __('Takentraining'); ?>']
,			['Employee.name', '<?php __('Employee'); ?>']
,			['Position.name', '<?php __('Position'); ?>']
,			['Branch.name', '<?php __('Branch'); ?>']
,			['Stafftooktraining.created', '<?php __('Created'); ?>']
,			['Stafftooktraining.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var stafftooktrainingSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					stafftooktrainingSearchWindow.close();
...
var stafftooktrainingSearchWindow = new Ext.Window({
	title: '<?php __('Search Stafftooktraining'); ?>',
	modal: true,
	items: stafftooktrainingSearchForm,
	resizable: false,
	width: 550,
	height: 400,
