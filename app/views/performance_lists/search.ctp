...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['PerformanceList.name', '<?php __('Name'); ?>']
,			['PerformanceList.type', '<?php __('Type'); ?>']
...
var performanceListSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					performanceListSearchWindow.close();
...
var performanceListSearchWindow = new Ext.Window({
	title: '<?php __('Search PerformanceList'); ?>',
	modal: true,
	items: performanceListSearchForm,
	resizable: false,
	width: 550,
	height: 400,
