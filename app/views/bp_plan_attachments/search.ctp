...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Plan.name', '<?php __('Plan'); ?>']
,			['BpPlanAttachment.file_name', '<?php __('File Name'); ?>']
,			['BpPlanAttachment.path', '<?php __('Path'); ?>']
,			['BpPlanAttachment.created', '<?php __('Created'); ?>']
,			['BpPlanAttachment.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var bpPlanAttachmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					bpPlanAttachmentSearchWindow.close();
...
var bpPlanAttachmentSearchWindow = new Ext.Window({
	title: '<?php __('Search BpPlanAttachment'); ?>',
	modal: true,
	items: bpPlanAttachmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,
