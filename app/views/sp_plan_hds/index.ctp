...
var store_spPlanHds = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'list_data')); ?>'
	})
});
...
function EditSpPlanHd(id) {
	...
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'edit1')); ?>/'+id,
		success: function(response, opts) {
			var spPlanHd_data = response.responseText;
...
function ReturnSpPlanHd(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'ret')); ?>/'+id,
		success: function(response, opts) {
			var spPlanHd_data = response.responseText;
...
if(center_panel.find('id', 'spPlanHd-tab') != "") {
	var p = center_panel.findById('spPlanHd-tab');
	...
	var p = center_panel.add({
		...
		columns: [
			...
			{header: "<?php __('Budget Center'); ?>", dataIndex: 'branch', sortable: true},
	...
	listeners: {
			celldblclick: function(){
				ViewParentSpPlans(Ext.getCmp('spPlanHd-tab').getSelectionModel().getSelected().data.id);
			}
		},
