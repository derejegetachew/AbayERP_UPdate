...
var store_spPlanHds = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'list_data1')); ?>'
	})
});

function CloseBudgetYear() {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'close')); ?>',
		success: function(response, opts) {
			Ext.Msg.alert('<?php __('Success'); ?>', '<?php __('Current Budget Year Closed!'); ?>');
			RefreshSpPlanHdData();
		},
		failure: function(response, opts) {
			Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot close current budget year. Error code'); ?>: ' + response.status);
		}
	});
}
...
function EditSpPlanHd(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'edit3')); ?>/'+id,
		success: function(response, opts) {
			var spPlanHd_data = response.responseText;
...
function ApproveSpPlanHd(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'approve')); ?>/'+id,
		success: function(response, opts) {
...
function ReturnSpPlanHd(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'finalret')); ?>/'+id,
		success: function(response, opts) {
			var spPlanHd_data = response.responseText;
...
if(center_panel.find('id', 'spPlanHd-tab3') != "") {
	var p = center_panel.findById('spPlanHd-tab3');
	center_panel.setActiveTab(p);
} else {
	var p = center_panel.add({
		title: '<?php __('Follow Up'); ?>',
		closable: true,
		loadMask: true,
		stripeRows: true,
		id: 'spPlanHd-tab3',
		...
		columns: [
			...
			{header: "<?php __('Budget Yearer'); ?>", dataIndex: 'budget_year', sortable: true},
		],
...
,
		listeners: {
			celldblclick: function(){
				ViewParentSpPlans(Ext.getCmp('spPlanHd-tab3').getSelectionModel().getSelected().data.id);
		...
		tbar: new Ext.Toolbar({
			...
			items: [ {
					...
					xtype: 'tbbutton',
					text: '<?php __('Close Current Budget Year'); ?>',
					handler: function(btn) {
						
							Ext.Msg.show({
									title: '<?php __('Close Budget Center'); ?>',
									buttons: Ext.MessageBox.YESNO,
									msg: '<?php __('Close Current Budget Year'); ?>',
									icon: Ext.MessageBox.QUESTION,
									fn: function(btn){
										if (btn == 'yes'){
										  CloseBudgetYear();
						...
						items: [
{
					...
					icon: 'img/table_view.png',
							cls: 'x-btn-text-icon',
					handler: function(btn) {
						window.open("http://10.1.85.85:8080/iportal/frameset?__report=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs%5CPlan_Detail_all.rptdesign&__format=html&__svg=true&__locale=en_US&__timezone=EAT&__masterpage=true&__rtl=false&__cubememsize=10&__resourceFolder=C%3A%5CUsers%5Cmhenoke%5Cworkspace%5CStrategyPlan&1142502393", "_blank");
					}
				},{
					...
					icon: 'img/table_view.png',
							cls: 'x-btn-text-icon',
					handler: function(btn) {
						window.open("http://10.1.85.85:8080/iportal/frameset?__report=C%3A%5CUsers%5CAdministrator%5CWorkspace%5CAbay+Report%5CReport+Designs%5Citem_by_budget_center.rptdesign&__format=html&__svg=true&__locale=en_US&__timezone=EAT&__masterpage=true&__rtl=false&__cubememsize=10&__resourceFolder=C%3A%5CUsers%5Cmhenoke%5Cworkspace%5CStrategyPlan&1810435176", "_blank");
					}
				}
	...
	p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
		...
		p.getTopToolbar().findById('approve-spPlanHd').enable();
		...
		if(this.getSelections().length > 1){
			...
			p.getTopToolbar().findById('approve-spPlanHd').disable();
