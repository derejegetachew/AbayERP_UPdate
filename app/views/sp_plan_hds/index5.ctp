...
var store_spPlanHds = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'spPlanHds', 'action' => 'list_data5')); ?>'
	})
});
...
function ViewParentSpPlans(id) {
    Ext.Ajax.request({
        url: '<?php echo $this->Html->url(array('controller' => 'spPlans', 'action' => 'index3')); ?>/'+id,
        success: function(response, opts) {
...
if(center_panel.find('id', 'spPlanHd-tab5') != "") {
	var p = center_panel.findById('spPlanHd-tab5');
...
,
		listeners: {
			celldblclick: function(){
				ViewParentSpPlans(Ext.getCmp('spPlanHd-tab5').getSelectionModel().getSelected().data.id);
