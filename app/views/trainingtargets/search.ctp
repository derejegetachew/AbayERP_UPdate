...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Trainingtarget.name', '<?php __('Name'); ?>']
,			['Trainingtarget.created', '<?php __('Created'); ?>']
,			['Trainingtarget.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var trainingtargetSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					trainingtargetSearchWindow.close();
...
var trainingtargetSearchWindow = new Ext.Window({
	title: '<?php __('Search Trainingtarget'); ?>',
	modal: true,
	items: trainingtargetSearchForm,
	resizable: false,
	width: 550,
	height: 400,
