...
<?php 
// $str = strval($e_id)."&".strval($budget_year_id)."&".strval($quarter);
// $str = "hello";
?>
...
function agree_technical(id){
	...
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'hoPerformancePlans', 'action' => 'agree_technical_br')); ?>/'+str,
...
if(center_panel.find('id', 'mybrsumObjectiveReport-tab') != "") {
	var p = center_panel.findById('mybrsumObjectiveReport-tab');
	center_panel.setActiveTab(p);
} else { 
	var p = center_panel.add({
	title: 'my technical report',
	id: 'mybrsumObjectiveReport-tab',
	closable: true,
