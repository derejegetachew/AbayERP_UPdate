...
var store_hoPerformancePlans = new Ext.data.GroupingStore({
	reader: new Ext.data.JsonReader({
		...
		fields: [
			'id','identifier','budget_year','quarter', 'employee',
				 
			]
	}),
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'hoPerformancePlans', 'action' => 'br_my_performance_history_list')); ?>'
	})
...
function my_tracking_report(id){
	myMask.show();
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'hoPerformancePlans', 'action' => 'br_my_objectives_report')); ?>/'+id,
...
function my_technical_report(id){
	myMask.show();
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'hoPerformancePlans', 'action' => 'br_my_sum_objectives_report')); ?>/'+id,
		success: function(response, opts) {
			var hoPerformancePlan_data = response.responseText;
...
function my_behavioural_report(id){
	myMask.show();
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'hoPerformancePlans', 'action' => 'br_my_behavioural_report')); ?>/'+id,
		success: function(response, opts) {
			var hoPerformancePlan_data = response.responseText;
...
function showMenu(grid, index, event) {
        ...
        var menu = new Ext.menu.Menu({
            items: [
				{
                    text: '<b>My Tracking Report </b>',
                    icon: 'img/table_add.png',
                    handler: function() {
						my_tracking_report(record.get('identifier'));
				...
				,
                {
                    ...
                    icon: 'img/table_add.png',
                    handler: function() {
						my_technical_report(record.get('identifier'));
				...
				,{
                    ...
                    icon: 'img/table_add.png',
                    handler: function() {
                       my_behavioural_report(record.get('identifier'));
...
if(center_panel.find('id', 'mybrPerformanceHistory-tab') != "") {
	var p = center_panel.findById('mybrPerformanceHistory-tab');
