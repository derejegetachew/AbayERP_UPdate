...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['HoPerformancePlan.name', '<?php __('Name'); ?>']
,			['BudgetYear.name', '<?php __('BudgetYear'); ?>']
,			['Employee.name', '<?php __('Employee'); ?>']
,			['HoPerformancePlan.quarter', '<?php __('Quarter'); ?>']
,			['HoPerformancePlan.agreed', '<?php __('Agreed'); ?>']
,			['HoPerformancePlan.comment', '<?php __('Comment'); ?>']
		]
	}),
	mode: 'local',
...
var hoPerformancePlanSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					hoPerformancePlanSearchWindow.close();
...
var hoPerformancePlanSearchWindow = new Ext.Window({
	title: '<?php __('Search HoPerformancePlan'); ?>',
	modal: true,
	items: hoPerformancePlanSearchForm,
	resizable: false,
	width: 550,
	height: 400,
