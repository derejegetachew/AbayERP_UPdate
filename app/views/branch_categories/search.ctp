...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['BranchCategory.name', '<?php __('Name'); ?>']
,			['BranchCategory.created', '<?php __('Created'); ?>']
,			['BranchCategory.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var branchCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					branchCategorySearchWindow.close();
...
var branchCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search BranchCategory'); ?>',
	modal: true,
	items: branchCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,
