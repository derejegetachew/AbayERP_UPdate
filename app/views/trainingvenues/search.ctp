...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Trainingvenue.name', '<?php __('Name'); ?>']
,			['Trainingvenue.address', '<?php __('Address'); ?>']
,			['Trainingvenue.created', '<?php __('Created'); ?>']
,			['Trainingvenue.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var trainingvenueSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					trainingvenueSearchWindow.close();
...
var trainingvenueSearchWindow = new Ext.Window({
	title: '<?php __('Search Trainingvenue'); ?>',
	modal: true,
	items: trainingvenueSearchForm,
	resizable: false,
	width: 550,
	height: 400,
