...
,			['CompetenceResult.quarter', '<?php __('Quarter'); ?>']
,			['Employee.name', '<?php __('Employee'); ?>']
,			['Competence.name', '<?php __('Competence'); ?>']
,			['CompetenceResult.actual_competence', '<?php __('Actual Competence'); ?>']
,			['CompetenceResult.score', '<?php __('Score'); ?>']
,			['CompetenceResult.rating', '<?php __('Rating'); ?>']
		]
	}),
	mode: 'local',
...
var competenceResultSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					competenceResultSearchWindow.close();
...
var competenceResultSearchWindow = new Ext.Window({
	title: '<?php __('Search CompetenceResult'); ?>',
	modal: true,
	items: competenceResultSearchForm,
	resizable: false,
	width: 550,
	height: 400,
