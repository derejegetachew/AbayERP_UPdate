...
,			['Grade.name', '<?php __('Grade'); ?>']
,			['Step.name', '<?php __('Step'); ?>']
,			['Position.name', '<?php __('Position'); ?>']
,			['EmployeeDetail.start_date', '<?php __('Start Date'); ?>']
,			['EmployeeDetail.created', '<?php __('Created'); ?>']
,			['EmployeeDetail.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var employeeDetailSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					employeeDetailSearchWindow.close();
...
var employeeDetailSearchWindow = new Ext.Window({
	title: '<?php __('Search EmployeeDetail'); ?>',
	modal: true,
	items: employeeDetailSearchForm,
	resizable: false,
	width: 550,
	height: 400,
