...
,			['PayrollEmployee.status', '<?php __('Status'); ?>']
,			['PayrollEmployee.date', '<?php __('Date'); ?>']
,			['PayrollEmployee.remark', '<?php __('Remark'); ?>']
		]
	}),
	mode: 'local',
...
var payrollEmployeeSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					payrollEmployeeSearchWindow.close();
...
var payrollEmployeeSearchWindow = new Ext.Window({
	title: '<?php __('Search PayrollEmployee'); ?>',
	modal: true,
	items: payrollEmployeeSearchForm,
	resizable: false,
	width: 550,
	height: 400,
