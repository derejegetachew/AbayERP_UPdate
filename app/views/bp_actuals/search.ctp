...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['BpActual.amount', '<?php __('Amount'); ?>'],	
			['BpActual.month', '<?php __('Month'); ?>'],	
			['Branch.name', '<?php __('Branch'); ?>'],
			['BpActual.remark', '<?php __('Remark'); ?>'],	
			['BpActual.type', '<?php __('Type'); ?>'],	
			['BpActual.created', '<?php __('Created'); ?>'],
			['BpActual.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var bpActualSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
...
var bpActualSearchWindow = new Ext.Window({
	title: '<?php __('Search BpActual'); ?>',
	modal: true,
	items: bpActualSearchForm,
	resizable: false,
	width: 550,
	height: 400,
