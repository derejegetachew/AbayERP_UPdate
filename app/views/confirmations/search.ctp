...
,			['Confirmation.confirmation_code', '<?php __('Confirmation Code'); ?>']
,			['Confirmation.status', '<?php __('Status'); ?>']
		]
	}),
	mode: 'local',
...
var confirmationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					confirmationSearchWindow.close();
...
var confirmationSearchWindow = new Ext.Window({
	title: '<?php __('Search Confirmation'); ?>',
	modal: true,
	items: confirmationSearchForm,
	resizable: false,
	width: 550,
	height: 400,
