...
var store_ibdDocuments = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'ibdDocuments', 'action' => 'list_data1')); ?>'
	})
...
function ViewIbdDocument(controller,action) {
    ...
    Ext.Ajax.request({
       ...
       url: 'http://'+ ip +'/AbayERP/'+controller+'/'+action,
        success: function(response, opts) {
            var ibdDocument_data = response.responseText;
...
if(center_panel.find('id', 'ibdDocument-tab-1') != "") {
	var p = center_panel.findById('ibdDocument-tab-1');
	center_panel.setActiveTab(p);
} else {
	var p = center_panel.add({
		title: '<?php __('Export Registration'); ?>',
		closable: true,
		loadMask: true,
		stripeRows: true,
		id: 'ibdDocument-tab-1',
		xtype: 'grid',
...
,
		listeners: {
			celldblclick: function(){
				ViewIbdDocument(Ext.getCmp('ibdDocument-tab-1').getSelectionModel().getSelected().data.controller,Ext.getCmp('ibdDocument-tab-1').getSelectionModel().getSelected().data.action);
			}
		},
