...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['CompetenceLevel.name', '<?php __('Name'); ?>']
		]
	}),
	mode: 'local',
...
var competenceLevelSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					competenceLevelSearchWindow.close();
...
var competenceLevelSearchWindow = new Ext.Window({
	title: '<?php __('Search CompetenceLevel'); ?>',
	modal: true,
	items: competenceLevelSearchForm,
	resizable: false,
	width: 550,
	height: 400,
