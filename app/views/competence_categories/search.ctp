...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['CompetenceCategory.name', '<?php __('Name'); ?>']
		]
	}),
	mode: 'local',
...
var competenceCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					competenceCategorySearchWindow.close();
...
var competenceCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search CompetenceCategory'); ?>',
	modal: true,
	items: competenceCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,
