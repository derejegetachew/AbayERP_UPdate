<?php
	...
	var ChangeStatusForm = new Ext.form.FormPanel({
			...
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'competenceCategories', 'action' => 'change_status_branch_hr')); ?>',
			...
			items: [
				...
				<?php 
					...
					$options['value'] = $competence_category['BranchPerformancePlan']['plan_status'];
				...
				<?php 
					...
					$options['value'] = $competence_category['BranchPerformancePlan']['result_status'];
					$this->ExtForm->input('result_status', $options);
				?>,
				<?php 
					
					$options = array();
					$options['value'] = $competence_category['BranchPerformancePlan']['comment'];
