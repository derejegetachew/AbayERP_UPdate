var settlment_store = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php  if(sizeof($ibdSettelments)>0){$id=$ibdSettelments[0]['IbdIbcs']['PURCHASE_ORDER_NO'];}else{$id="-1";} echo $this->Html->url(array('controller' => 'ibdSettelments', 'action' => 'list_data_po',str_replace("/",")", $id))); ?>'	})
});
  ...
  $total_fcy=0;
   $total_lcy=0;
 	foreach($ibdSettelments as $s){
 		$total_fcy+=$s['IbdIbcs']['SETT_FCY'];
 		$total_lcy+=$s['IbdIbcs']['SETT_Amount'];
 	}

 $ibdSettelment_html = "<table cellspacing=3>" . 		"<tr><th align=right>" . __('PO Number', true) . ":</th><td><b>" . $ibdSettelments[0]['IbdIbcs']['PURCHASE_ORDER_NO'] . "</b></td></tr>" . 
		"<tr><th align=right>" . __('Total Settled FCT Amount', true) . ":</th><td><b>" . $total_fcy . "</b></td></tr>". 
		"<tr><th align=right>" . __('Total Settled LCY Amount', true) . ":</th><td><b>" . $total_lcy . "</b></td></tr>". 
...
?>
		...
		var IbdSettelmentViewWindow = new Ext.Window({
			title: '<?php __('View IbdSettelment'); ?>: <?php if(sizeof($ibdSettelments)>0) {echo $ibdSettelments[0]['IbdIbcs']['id'];} ?>',
			width: 600,
			height:400,
			minWidth: 500,
			minHeight: 345,
