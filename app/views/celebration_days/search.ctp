...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['CelebrationDay.day', '<?php __('Day'); ?>']
,			['CelebrationDay.name', '<?php __('Name'); ?>']
...
var celebrationDaySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					celebrationDaySearchWindow.close();
...
var celebrationDaySearchWindow = new Ext.Window({
	title: '<?php __('Search CelebrationDay'); ?>',
	modal: true,
	items: celebrationDaySearchForm,
	resizable: false,
	width: 550,
	height: 400,
