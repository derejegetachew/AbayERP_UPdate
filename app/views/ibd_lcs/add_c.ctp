		...
		var IbdIbcAddForm = new Ext.form.FormPanel({
			...
			labelWidth: 180,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'IbdLcs', 'action' => 'add_c')); ?>',
			...
			items: [
			  {  
				    ...
				    name: 'data[IbdLc][LC_REF_NO]', 
				    fieldLabel: 'LC REFERENCE', 
				    typeAhead: true, 
					...
					Ext.Ajax.request({
                         url:'<?php echo $this->Html->url(array('controller' => 'ibdLcs', 'action' => 'get_lc_detail')); ?>/'+permit,
                         success:function(data,opts){
                            var jsonData = Ext.util.JSON.decode(data.responseText);
							console.log(jsonData);
                           
                            Ext.getCmp('NAME_OF_IMPORTER').setValue(jsonData[0]['ibd_lcs']['NAME_OF_IMPORTER']);
                            Ext.getCmp('currency').setValue(jsonData[0]['ibd_lcs']['currency_id']);
                            Ext.getCmp('SETT_FCY').setValue(jsonData[0]['ibd_lcs']['OUT_FCY_AMOUNT']);
                            remaining=jsonData[0]['ibd_lcs']['OUT_FCY_AMOUNT'];
