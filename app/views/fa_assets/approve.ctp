...
var store_faAssets = new Ext.data.GroupingStore({
	reader: new Ext.data.JsonReader({
		...
		fields: [
			'id','reference','name','location','original_cost','book_date','sold','sold_date','tax_rate','tax_cat','ifrs_class','ifrs_cat','ifrs_useful_age','residual_value_rate']
	}),
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'faAssets', 'action' => 'list_data_a')); ?>'
	})
...
function ApproveFaAsset(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'faAssets', 'action' => 'approveasset')); ?>/'+id,
		success: function(response, opts) {
...
if(center_panel.find('id', 'faAsset-tab-approveb') != "") {
	var p = center_panel.findById('faAsset-tab-approveb');
	center_panel.setActiveTab(p);
} else {
	var p = center_panel.add({
		title: '<?php __('Approve Asset'); ?>',
		closable: true,
		loadMask: true,
		stripeRows: true,
		id: 'faAsset-tab-approve',
		xtype: 'grid',
...
,
		listeners: {
			celldblclick: function(){
				ViewFaAsset(Ext.getCmp('faAsset-tab-approve').getSelectionModel().getSelected().data.id);
			}
		},
