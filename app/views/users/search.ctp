...
function AddParameter(field, value){
	...
	if(currentlyToggled == "IN"){
		...
		row = [[field, 'array("University.name BETWEEN ? AND ?" => array( ' + InsertQuotations(value) + ' ))']];
	}
...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['User.username', '<?php __('Username'); ?>']
,			['User.password', '<?php __('Password'); ?>']
,			['User.email', '<?php __('Email'); ?>']
,			['User.is_active', '<?php __('Is Active'); ?>']
,			['User.security_question', '<?php __('Security Question'); ?>']
,			['User.security_answer', '<?php __('Security Answer'); ?>']
,			['Person.name', '<?php __('Person'); ?>']
,			['User.created', '<?php __('Created'); ?>']
,			['User.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var userSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
