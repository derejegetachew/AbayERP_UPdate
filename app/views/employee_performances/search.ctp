...
,			['Performance.name', '<?php __('Performance'); ?>']
,			['EmployeePerformance.status', '<?php __('Status'); ?>']
,			['EmployeePerformance.created', '<?php __('Created'); ?>']
		]
	}),
	mode: 'local',
...
var employeePerformanceSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					employeePerformanceSearchWindow.close();
...
var employeePerformanceSearchWindow = new Ext.Window({
	title: '<?php __('Search EmployeePerformance'); ?>',
	modal: true,
	items: employeePerformanceSearchForm,
	resizable: false,
	width: 550,
	height: 400,
