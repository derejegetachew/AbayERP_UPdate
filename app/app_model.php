    ...
    function  beforeValidate($options = array()) {
        ...
        foreach ($this->validate as $distinctFieldKey => $distinctFieldValue) {
            foreach ($distinctFieldValue as $customRuleKey => $customRuleValue) {
			 ...
			 if (strtolower($ruleValue[0]) == 'date' && array_key_exists($distinctFieldKey, $this->data[$this->name])) {
                            $tempdate=explode('/', $this->data[$this->name][$distinctFieldKey]);
							if(count($tempdate)>=2)
                            $this->data[$this->name][$distinctFieldKey] = $tempdate[2].'-'.$tempdate[1].'-'.$tempdate[0];
                        }
