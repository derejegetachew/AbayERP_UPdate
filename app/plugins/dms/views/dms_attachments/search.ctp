...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['DmsAttachment.name', '<?php __('Name'); ?>']
,			['DmsAttachment.file', '<?php __('File'); ?>']
,			['DmsMessage.name', '<?php __('DmsMessage'); ?>']
,			['DmsAttachment.created', '<?php __('Created'); ?>']
		]
	}),
	mode: 'local',
...
var dmsAttachmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					dmsAttachmentSearchWindow.close();
...
var dmsAttachmentSearchWindow = new Ext.Window({
	title: '<?php __('Search DmsAttachment'); ?>',
	modal: true,
	items: dmsAttachmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,
