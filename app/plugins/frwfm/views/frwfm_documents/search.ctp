...
,			['FrwfmDocument.name', '<?php __('Name'); ?>']
,			['FrwfmDocument.file_path', '<?php __('File Path'); ?>']
,			['FrwfmDocument.created', '<?php __('Created'); ?>']
		]
	}),
	mode: 'local',
...
var frwfmDocumentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					frwfmDocumentSearchWindow.close();
...
var frwfmDocumentSearchWindow = new Ext.Window({
	title: '<?php __('Search FrwfmDocument'); ?>',
	modal: true,
	items: frwfmDocumentSearchForm,
	resizable: false,
	width: 550,
	height: 400,
