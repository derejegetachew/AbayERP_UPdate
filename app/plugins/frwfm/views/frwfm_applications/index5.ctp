...
var store_frwfmApplications = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'frwfmApplications', 'action' => 'list_data5')); ?>'
	})
});
...
function Update(id) {
	Ext.MessageBox.show({
		  msg: 'Updating from FlexCube, please wait...',
		  progressText: 'Saving...',
		  width:300,
		  wait:true,
	...
	Ext.Ajax.request({
		...
		success: function(response, opts) {
			Ext.MessageBox.hide(); 
			Ext.Msg.alert('<?php __('Success'); ?>', '<?php __('Update Completed'); ?>');
			RefreshFrwfmApplicationData();
		},
		failure: function(response, opts) {
...
if(center_panel.find('id', 'frwfmApplication-tab5') != "") {
	var p = center_panel.findById('frwfmApplication-tab5');
	center_panel.setActiveTab(p);
} else {
	var p = center_panel.add({
		title: '<?php __('View All Requests'); ?>',
		closable: true,
		loadMask: true,
		stripeRows: true,
		id: 'frwfmApplication-tab5',
		xtype: 'grid',
...
,
		listeners: {
			celldblclick: function(){
				ViewFrwfmApplication(Ext.getCmp('frwfmApplication-tab5').getSelectionModel().getSelected().data.id);
			}
		},
		...
		tbar: new Ext.Toolbar({
			...
			items: [ {
						 ...
						 {
							text: '<?php __('Update Accounts'); ?>',
                            icon: 'img/table_view.png',
							cls: 'x-btn-text-icon',
							id: 'update-frwfmApplication',
							handler: function(btn) {
							Ext.Msg.show({
									title: '<?php __('Update'); ?>',
									buttons: Ext.MessageBox.YESNO,
									msg: '<?php __('Update All Current Bank Account Balances'); ?> ',
