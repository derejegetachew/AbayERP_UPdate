...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['FrwfmApplication.name', '<?php __('FrwfmApplication'); ?>']
,			['FrwfmAccount.acc_no', '<?php __('Acc No'); ?>']
,			['FrwfmAccount.name', '<?php __('Name'); ?>']
,			['FrwfmAccount.branch', '<?php __('Branch'); ?>']
,			['FrwfmAccount.amount', '<?php __('Amount'); ?>']
,			['FrwfmAccount.currency', '<?php __('Currency'); ?>']
,			['FrwfmAccount.type', '<?php __('Type'); ?>']
		]
	}),
	mode: 'local',
...
var frwfmAccountSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					frwfmAccountSearchWindow.close();
...
var frwfmAccountSearchWindow = new Ext.Window({
	title: '<?php __('Search FrwfmAccount'); ?>',
	modal: true,
	items: frwfmAccountSearchForm,
	resizable: false,
	width: 550,
	height: 400,
