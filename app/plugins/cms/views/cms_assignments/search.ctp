...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['CmsCase.name', '<?php __('CmsCase'); ?>']
,			['CmsAssignment.assigned_by', '<?php __('Assigned By'); ?>']
,			['CmsAssignment.assigned_to', '<?php __('Assigned To'); ?>']
,			['CmsAssignment.created', '<?php __('Created'); ?>']
		]
	}),
	mode: 'local',
...
var cmsAssignmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					cmsAssignmentSearchWindow.close();
...
var cmsAssignmentSearchWindow = new Ext.Window({
	title: '<?php __('Search CmsAssignment'); ?>',
	modal: true,
	items: cmsAssignmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,
