...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['CmsAttachment.file', '<?php __('File'); ?>']
,			['CmsAttachment.name', '<?php __('Name'); ?>']
,			['CmsAttachment.created', '<?php __('Created'); ?>']
,			['CmsReply.name', '<?php __('CmsReply'); ?>']
		]
	}),
	mode: 'local',
...
var cmsAttachmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					cmsAttachmentSearchWindow.close();
...
var cmsAttachmentSearchWindow = new Ext.Window({
	title: '<?php __('Search CmsAttachment'); ?>',
	modal: true,
	items: cmsAttachmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,
