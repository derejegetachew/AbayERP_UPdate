//<script>
	...
	reader: new Ext.data.JsonReader({
            ...
            fields: [
                'id','purchase_order','item','measurement','ordered_quantity','purchased_quantity','unit_price','remark','created','modified','total_price'	
            ]
	}),
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_order_items', 'action' => 'list_data', $parent_id)); ?>'	})
    });
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_order_items', 'action' => 'add', $parent_id)); ?>',
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_order_items', 'action' => 'edit')); ?>/'+id+'/<?php echo $parent_id; ?>',
            success: function(response, opts) {
                var parent_purchaseOrderItem_data = response.responseText;
    ...
    store_parent_purchaseOrderItems.setBaseParam('query', filter_conditions);
	...
	title: '<?php __('Purchase Order Items    [Grand Total Price = <span style="color:#1E90FF">'.number_format($grand_total_price, 2 , '.' , ',' ).'</span>]'); ?>',
    ...
    g.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
        ...
        else if(<?php echo $posted; ?> ==1){
            g.getTopToolbar().findById('add-parent-purchaseOrderItem').disable();
        }
