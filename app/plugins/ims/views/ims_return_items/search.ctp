...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ImsReturn.name', '<?php __('ImsReturn'); ?>']
,			['ImsSirvItem.name', '<?php __('ImsSirvItem'); ?>']
,			['ImsReturnItem.quantity', '<?php __('Quantity'); ?>']
,			['ImsReturnItem.remark', '<?php __('Remark'); ?>']
,			['ImsReturnItem.created', '<?php __('Created'); ?>']
,			['ImsReturnItem.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var imsReturnItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					imsReturnItemSearchWindow.close();
...
var imsReturnItemSearchWindow = new Ext.Window({
	title: '<?php __('Search ImsReturnItem'); ?>',
	modal: true,
	items: imsReturnItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,
