...
var store_imsSirvs = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'list_data')); ?>'
	})
...
function AddImsSirv() {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'add')); ?>',
...
function EditImsSirv(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'edit')); ?>/'+id,
		success: function(response, opts) {
			var imsSirv_data = response.responseText;
...
function DeleteImsSirv(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'delete')); ?>/'+id,
		success: function(response, opts) {
			Ext.Msg.alert('<?php __('Success'); ?>', '<?php __('ImsSirv successfully deleted!'); ?>');
			RefreshImsSirvData();
		},
		failure: function(response, opts) {
			Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the imsSirv add form. Error code'); ?>: ' + response.status);
...
function PrintSIRV(id) {
        url = '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'print_sirv_copy')); ?>/' + id;	
       
        popUpWindow(url, 250, 20, 900, 600);
}

function AdjustSIRV(id) {
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_sirv_items', 'action' => 'index_5')); ?>/'+id,
            success: function(response, opts) {
...
function CreateTag(id) {
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_sirv_items', 'action' => 'index_6')); ?>/'+id,
            success: function(response, opts) {
                var sirv_item_data = response.responseText;
...
function Transfer(id) {
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_sirv_items', 'action' => 'index_7')); ?>/'+id,
            success: function(response, opts) {
                var sirv_item_data = response.responseText;
...
function Return(id) {
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_sirv_items', 'action' => 'index_8')); ?>/'+id,
            success: function(response, opts) {
                var sirv_item_data = response.responseText;
...
function showMenu(grid, index, event) {
		...
		<?php foreach($groups as $group){
			...
			?>
				btnStatus1 = (record.get('status') == 'completed')? false: true;
				btnStatus2 = (record.get('status') == 'completed')? false: true;
				btnStatus3 = (record.get('status') == 'completed')? false: true;
			<?php
			}
			else if ($group['name'] == 'Stock Record Officer'){
			?>
				btnStatus2 = (record.get('status') == 'completed')? false: true;				
			<?php
			}
        ...
        var menu = new Ext.menu.Menu({
            items: [
				...
				{
                    text: '<b>Adjust SIRV</b>',
                    icon: 'img/table_edit.png',
                    handler: function() {
                        AdjustSIRV(record.get('id'));
                    },
                    disabled: btnStatus1
                },
				{
...
if(center_panel.find('id', 'imsSirv-tab') != "") {
	var p = center_panel.findById('imsSirv-tab');
...
,
		listeners: {
			celldblclick: function(){
				ViewImsSirv(Ext.getCmp('imsSirv-tab').getSelectionModel().getSelected().data.id);
			},
						...
						items: [
{
					...
					store : new Ext.data.ArrayStore({
						...
						data : [
							['-1', 'All'],
							<?php $st = false;foreach ($ims_requisitions as $item){if($st) echo ",
