...
var store_imsSirvs = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvs', 'action' => 'list_data1')); ?>'
	})
...
if(center_panel.find('id', 'imsSirv-tab1') != "") {
	var p = center_panel.findById('imsSirv-tab1');
...
,
		listeners: {
			celldblclick: function(){
				PrintSIRV(Ext.getCmp('imsSirv-tab1').getSelectionModel().getSelected().data.id);
			},
