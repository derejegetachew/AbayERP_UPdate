...
var store_parent_imsSirvItems = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvItemBefores', 'action' => 'list_sirv_items_data3', $sirv_id)); ?>'	})
});
...
function Return(){
		...
		if(from_branch != '' && from_user != ''){
			...
			Ext.Ajax.request({
				url: '<?php echo $this->Html->url(array('controller' => 'ims_returns', 'action' => 'add_return_before', $sirv_id)); ?>/'+from_branch+'/'+from_user+'/'+number,
				params:param,
				method: 'POST',				
