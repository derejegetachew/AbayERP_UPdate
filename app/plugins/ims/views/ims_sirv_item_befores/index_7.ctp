...
var store_parent_imsSirvItems = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'imsSirvItemBefores', 'action' => 'list_sirv_items_data2', $sirv_id)); ?>'	})
});
...
function RefreshParentImsSirvItemData() {
  ...
  var editor = new Ext.ux.grid.RowEditor({
	...
	function Transfer(){
		...
		//my code @melkamu //
		var to_branch = "from"; //"this value   will get from recivers branch";
		var from_branch = "to";  //"this value will get from from user branch";
		//end//
		...
		if(from_branch != '' && to_branch != '' && from_user != '' && to_user != ''){
			...
			Ext.Ajax.request({
				url: '<?php echo $this->Html->url(array('controller' => 'ims_transfer_befores', 'action' => 'add_transfer', $sirv_id)); ?>/'+from_branch+'/'+to_branch+'/'+from_user+'/'+to_user+'/'+number,
				params:param,
				method: 'POST',				
				success: function(response, opts){
...
var g = new Ext.grid.GridPanel({
	title: '<?php __('SIRV Items Before'); ?>',
	...
	columns: [
		...
		{header: "<?php __('Tag'); ?>", dataIndex: 'tag', editor: { id:'data[tag]', allowBlank: false, xtype: 'textarea' }, sortable: true, align:'left'} 	],
...
var c = new Ext.form.FormPanel({
			...
			},
		{
			...
			// 	displayField: 'name',
			// 	typeAhead: true,
			// 	hiddenName:'data[ImsTransferBefore][from_branch]',
			// 	id: 'from_branch',
			// 	name: 'from_branch',
			// 	mode: 'local',					
			// 	triggerAction: 'all',
			...
			// 	displayField: 'name',
			// 	typeAhead: true,
			// 	hiddenName:'data[ImsTransferBefore][to_branch]',
			// 	id: 'to_branch',
			// 	name: 'to_branch',
			// 	mode: 'local',					
			// 	triggerAction: 'all',
			...
			// }
				...
				items:[
			 {
				...
				displayField: 'name',
				typeAhead: true,
				hiddenName:'data[ImsTransferBefore][from_user]',
				id: 'from_user',
				name: 'from_user',
				mode: 'local',					
				triggerAction: 'all',
				...
				displayField: 'name',
				typeAhead: true,
				hiddenName:'data[ImsTransferBefore][to_user]',
				id: 'to_user',
				name: 'to_user',
				mode: 'local',					
				triggerAction: 'all',
...
var parentImsSirvItemsViewWindow = new Ext.Window({
	title: 'SIRV Items before for transfer',
	width: 700,
	height:375,
	minWidth: 700,
	minHeight: 400,
