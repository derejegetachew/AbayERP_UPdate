...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ImsSirvBefore.name', '<?php __('Name'); ?>']
,			['Branch.name', '<?php __('Branch'); ?>']
,			['ImsSirvBefore.created', '<?php __('Created'); ?>']
,			['ImsSirvBefore.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var imsSirvBeforeSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					imsSirvBeforeSearchWindow.close();
...
var imsSirvBeforeSearchWindow = new Ext.Window({
	title: '<?php __('Search ImsSirvBefore'); ?>',
	modal: true,
	items: imsSirvBeforeSearchForm,
	resizable: false,
	width: 550,
	height: 400,
