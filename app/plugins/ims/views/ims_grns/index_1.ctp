//<script>
	...
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_grns', 'action' => 'list_data_1')); ?>'
	}),	
    ...
    function showMenu(grid, index, event) {
        ...
        var btnStatus = (record.get('status') == 'approved' || record.get('status') == 'rejected')? true: false;
        //alert(btnStatus);
		var btnStatus1 = true;
		<?php foreach($groups as $group){
			if($group['name'] == 'Stock Supervisor'){
			?>
				btnStatus1 = (record.get('status') == 'approved')? false: true;
			<?php
			}			
    ...
    if(center_panel.find('id', 'approve-grn-tab') != "") {
	var p = center_panel.findById('approve-grn-tab');
	center_panel.setActiveTab(p);
    } else {
	var p = center_panel.add({
            title: '<?php __('Approve GRNs'); ?>',
            closable: true,
            loadMask: true,
            stripeRows: true,
            id: 'approve-grn-tab',
            ...
            tbar: new Ext.Toolbar({
                items: [{
                        ...
                        text: '<?php __('Approve'); ?>',
                        id: 'grn-approve',
                        tooltip:'<?php __('<b>Approve GRNs</b><br />Click here to approve GRN'); ?>',
                        icon: 'img/table_edit.png',
                        cls: 'x-btn-text-icon',
                        disabled: true,
        ...
        if(r.get('status')=='created' || r.get('status')=='approved'){
            p.getTopToolbar().findById('grn-approve').disable();            
            ...
            p.getTopToolbar().findById('grn-approve').enable();
            p.getTopToolbar().findById('view-grn-approve').enable();
            if(this.getSelections().length > 1){
                p.getTopToolbar().findById('grn-approve').disable();
                p.getTopToolbar().findById('view-grn-approve').disable();
