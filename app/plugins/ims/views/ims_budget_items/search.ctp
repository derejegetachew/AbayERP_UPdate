...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ImsBudget.name', '<?php __('ImsBudget'); ?>']
,			['ImsItem.name', '<?php __('ImsItem'); ?>']
,			['ImsBudgetItem.quantity', '<?php __('Quantity'); ?>']
,			['ImsBudgetItem.measurement', '<?php __('Measurement'); ?>']
,			['ImsBudgetItem.created', '<?php __('Created'); ?>']
,			['ImsBudgetItem.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var imsBudgetItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					imsBudgetItemSearchWindow.close();
...
var imsBudgetItemSearchWindow = new Ext.Window({
	title: '<?php __('Search ImsBudgetItem'); ?>',
	modal: true,
	items: imsBudgetItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,
