    ...
    <?php
        $this->ExtForm->create('ImsItemCategory');
    ...
    var ItemCategoryEditForm = new Ext.form.FormPanel({
        ...
        labelWidth: 100,
        labelAlign: 'right',
        url:'<?php echo $this->Html->url(array('controller' => 'ims_item_categories', 'action' => 'edit')); ?>',
        defaultType: 'textfield',

        items: [
            <?php $this->ExtForm->input('id', array('hidden' => $item_category['ImsItemCategory']['id'])); ?>,
            <?php 
                $options = array();
                $options['value'] = $item_category['ImsItemCategory']['name'];
                $this->ExtForm->input('name', $options);
            ?>,
            <?php 
                $options = array();
                $options['hidden'] = $item_category['ImsItemCategory']['parent_id'];
