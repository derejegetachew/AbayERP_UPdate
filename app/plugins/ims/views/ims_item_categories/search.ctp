...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ItemCategory.name', '<?php __('Name'); ?>']
,			['ParentItemCategory.name', '<?php __('ParentItemCategory'); ?>']
,			['ItemCategory.lft', '<?php __('Lft'); ?>']
,			['ItemCategory.rght', '<?php __('Rght'); ?>']
,			['ItemCategory.created', '<?php __('Created'); ?>']
,			['ItemCategory.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var itemCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'itemCategories', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					itemCategorySearchWindow.close();
...
var itemCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search ItemCategory'); ?>',
	modal: true,
	items: itemCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,
