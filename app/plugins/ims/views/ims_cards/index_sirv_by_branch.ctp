//<script>
    ...
    var SIRVByBranchForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'ims_cards', 'action' => 'sirv_by_branch')); ?>',
    ...
    var SIRVByBranchWindow = new Ext.Window({
	title: 'Booked SIRV',
	width: 400,
	minWidth: 400,
	autoHeight: true,
	resizable: false,
	items: SIRVByBranchForm,
	plain:true,
	...
	tools: [{
				...
				qtip: 'Reset',
				handler: function () {
					SIRVByBranchForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					if(SIRVByBranchWindow.collapsed)
						SIRVByBranchWindow.expand(true);
					else
						SIRVByBranchWindow.collapse(true);
				}
			}],
	buttons: [{
		text: 'Display',
		handler: function(btn){
			var form = SIRVByBranchForm.getForm(); // or inputForm.getForm();
			...
			el.action = form.url;
			el.submit();
		//SIRVByBranchForm.getForm().submit({  target:'_blank'});
		}
            },{
		text: 'Close',
		handler: function(btn){
                    SIRVByBranchWindow.close();
		}
            }]
    });
	SIRVByBranchWindow.show();