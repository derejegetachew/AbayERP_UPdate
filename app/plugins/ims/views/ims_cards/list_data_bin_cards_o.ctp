...
<?php $st = false; $balance = 0;foreach($cards as $card){if($st) echo ","; ?>			{
			...
			"supplied_issued":"<?php 
				if($card['ImsCard']['ims_grn_item_id'] == null){
					...
					if($card['ImsCard']['disp_quantity'] != 0){
            ...
            "out_quantity":"<?php if($card['ImsCard']['out_quantity'] != 0){
									echo $card['ImsCard']['out_quantity'];
								} else if($card['ImsCard']['disp_quantity'] != 0){
									echo $card['ImsCard']['disp_quantity'];
								}else echo ''; ?>",
            "balance":"<?php if($card['ImsCard']['in_quantity'] != 0){
								$balance = sprintf("%.2f", $balance) + sprintf("%.2f", $card['ImsCard']['in_quantity']);
							}else if($card['ImsCard']['out_quantity'] != 0 ){
								$balance = sprintf("%.2f", $balance) - sprintf("%.2f", $card['ImsCard']['out_quantity']);
							}else if($card['ImsCard']['disp_quantity'] != 0 ){
								$balance = sprintf("%.2f", $balance) - sprintf("%.2f", $card['ImsCard']['disp_quantity']);
							}
				echo $balance; ?>",
			"initial":"<?php echo ''; ?>",
