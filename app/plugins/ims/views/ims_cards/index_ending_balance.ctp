//<script>
    ...
    var EndingBalanceForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'ims_cards', 'action' => 'display')); ?>',
    ...
    var EndingBalanceWindow = new Ext.Window({
	title: 'Ending Balance',
	width: 400,
	minWidth: 400,
	autoHeight: true,
	resizable: false,
	items: EndingBalanceForm,
	plain:true,
	...
	tools: [{
				...
				qtip: 'Reset',
				handler: function () {
					EndingBalanceForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					if(EndingBalanceWindow.collapsed)
						EndingBalanceWindow.expand(true);
					else
						EndingBalanceWindow.collapse(true);
				}
			}],
	buttons: [{
		text: 'Display',
		handler: function(btn){
			var form = EndingBalanceForm.getForm(); // or inputForm.getForm();
			...
			el.action = form.url;
			el.submit();
		//EndingBalanceForm.getForm().submit({  target:'_blank'});
		}
            },{
		text: 'Close',
		handler: function(btn){
                    EndingBalanceWindow.close();
		}
            }]
    });
	EndingBalanceWindow.show();