//<script>
    ...
    var ReturnedDisposalByCategoryForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'ims_cards', 'action' => 'returned_disposal_by_category')); ?>',
    ...
    var ReturnedDisposalByCategoryWindow = new Ext.Window({
	title: 'Returned Item Disposal',
	width: 400,
	minWidth: 400,
	autoHeight: true,
	resizable: false,
	items: ReturnedDisposalByCategoryForm,
	plain:true,
	...
	tools: [{
				...
				qtip: 'Reset',
				handler: function () {
					ReturnedDisposalByCategoryForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					if(ReturnedDisposalByCategoryWindow.collapsed)
						ReturnedDisposalByCategoryWindow.expand(true);
					else
						ReturnedDisposalByCategoryWindow.collapse(true);
				}
			}],
	buttons: [{
		text: 'Display',
		handler: function(btn){
			var form = ReturnedDisposalByCategoryForm.getForm(); // or inputForm.getForm();
			...
			el.action = form.url;
			el.submit();
		//ReturnedDisposalByCategoryForm.getForm().submit({  target:'_blank'});
		}
            },{
		text: 'Close',
		handler: function(btn){
                    ReturnedDisposalByCategoryWindow.close();
		}
            }]
    });
	ReturnedDisposalByCategoryWindow.show();