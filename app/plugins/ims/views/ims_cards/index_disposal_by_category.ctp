//<script>
    ...
    var DisposalByCategoryForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'ims_cards', 'action' => 'disposal_by_category')); ?>',
    ...
    var DisposalByCategoryWindow = new Ext.Window({
	title: 'Booked SIRV',
	width: 400,
	minWidth: 400,
	autoHeight: true,
	resizable: false,
	items: DisposalByCategoryForm,
	plain:true,
	...
	tools: [{
				...
				qtip: 'Reset',
				handler: function () {
					DisposalByCategoryForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					if(DisposalByCategoryWindow.collapsed)
						DisposalByCategoryWindow.expand(true);
					else
						DisposalByCategoryWindow.collapse(true);
				}
			}],
	buttons: [{
		text: 'Display',
		handler: function(btn){
			var form = DisposalByCategoryForm.getForm(); // or inputForm.getForm();
			...
			el.action = form.url;
			el.submit();
		//DisposalByCategoryForm.getForm().submit({  target:'_blank'});
		}
            },{
		text: 'Close',
		handler: function(btn){
                    DisposalByCategoryWindow.close();
		}
            }]
    });
	DisposalByCategoryWindow.show();