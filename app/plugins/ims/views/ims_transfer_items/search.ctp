...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ImsTransfer.name', '<?php __('ImsTransfer'); ?>']
,			['ImsItem.name', '<?php __('ImsItem'); ?>']
,			['ImsTransferItem.quantity', '<?php __('Quantity'); ?>']
,			['ImsTransferItem.unit_price', '<?php __('Unit Price'); ?>']
,			['ImsTransferItem.created', '<?php __('Created'); ?>']
,			['ImsTransferItem.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var imsTransferItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					imsTransferItemSearchWindow.close();
...
var imsTransferItemSearchWindow = new Ext.Window({
	title: '<?php __('Search ImsTransferItem'); ?>',
	modal: true,
	items: imsTransferItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,
