//<script>
	...
	reader: new Ext.data.JsonReader({
            ...
            fields: [
                'id','name','supplier','planned','user','posted','approved', 
                'rejected','created','modified'	
            ]
	}),
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'list_data_1')); ?>'
	}),	
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'approve')); ?>/'+id,
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'reject')); ?>/'+id,
            success: function(response, opts) {
            ...
            tbar: new Ext.Toolbar({
                ...
                items: [{
                        ...
                        handler: function(btn) {
                            ...
                            if (sm.hasSelection()){
                                Ext.Msg.show({
                                    title: '<?php __('Reject Purchase Order'); ?>',
                                    buttons: Ext.MessageBox.YESNO,
