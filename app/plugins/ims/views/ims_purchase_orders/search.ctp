...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ImsPurchaseOrder.name', '<?php __('Name'); ?>']
,			['User.name', '<?php __('User'); ?>']
,			['ImsPurchaseOrder.created', '<?php __('Created'); ?>']
,			['ImsPurchaseOrder.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var purchaseOrderSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'purchaseOrders', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					purchaseOrderSearchWindow.close();
...
var purchaseOrderSearchWindow = new Ext.Window({
	title: '<?php __('Search PurchaseOrder'); ?>',
	modal: true,
	items: purchaseOrderSearchForm,
	resizable: false,
	width: 550,
	height: 400,
