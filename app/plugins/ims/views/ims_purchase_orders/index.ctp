//<script>
	...
	reader: new Ext.data.JsonReader({
            ...
            fields: [
                'id','name','user','supplier','planned','approved_by','postable','posted','approved','rejected','completed','created','modified'		
            ]
	}),
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'list_data')); ?>'
	}),	
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'edit')); ?>/'+id,
            success: function(response, opts) {
                var purchaseOrder_data = response.responseText;
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'post')); ?>/'+id,
            success: function(response, opts) {
    ...
    function ViewParentPurchaseOrderItems(id,posted) {
        Ext.Ajax.request({
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'search')); ?>',
            success: function(response, opts){
                var purchaseOrder_data = response.responseText;
	...
	url = '<?php echo $this->Html->url(array('controller' => 'ims_purchase_orders', 'action' => 'print_po')); ?>/' + id;	
    ...
    function showMenu(grid, index, event) {
        ...
        var btnStatus = (record.get('posted') == '<font color=green>True</font>')? true: false;
		var btnApprove = (record.get('approved') == '<font color=green>True</font>')? false: true;
            ...
            items: [
                {
                    ...
                    icon: 'img/table_view.png',
                    handler: function() {
                        ViewParentPurchaseOrderItems(record.get('id'),record.get('posted'));
                    },
                    disabled: btnStatus
                }, {
	...
	var p = center_panel.add({
            ...
            columns: [
				...
				{header: "<?php __('Is Planned?'); ?>", dataIndex: 'planned', sortable: true},
    ...
    foreach ($users as $item) {
        if ($st)
            echo ",
							";
        ?>['<?php echo $item['User']['id']; ?>' ,'<?php echo $item['Person']['first_name'] .' '. $item['Person']['middle_name'] .' '.  $item['Person']['last_name']; ?>']<?php $st = true;
}
    ?>				]
                            }),
                            displayField : 'name',
    ...
    foreach ($suppliers as $item) {
        if ($st)
            echo ",
							";
        ?>['<?php echo $item['ImsSupplier']['id']; ?>' ,'<?php echo $item['ImsSupplier']['name']; ?>']<?php $st = true;
}
    ?>				]
                            }),
                            displayField : 'name',
