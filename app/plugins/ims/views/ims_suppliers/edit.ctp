		...
		var SupplierEditForm = new Ext.form.FormPanel({
			...
			labelWidth: 100,
			labelAlign: 'right',
			url:'<?php echo $this->Html->url(array('controller' => 'ims_suppliers', 'action' => 'edit')); ?>',
			defaultType: 'textfield',

			items: [
				<?php $this->ExtForm->input('id', array('hidden' => $supplier['ImsSupplier']['id'])); ?>,
				<?php 
					$options = array();
					$options['value'] = $supplier['ImsSupplier']['name'];
					$this->ExtForm->input('name', $options);
				?>,
				<?php 
					$options = array();
					$options['value'] = $supplier['ImsSupplier']['address'];
					$this->ExtForm->input('address', $options);
				?>,
				<?php 
					$options = array('fieldLabel' => 'TIN Number');
					$options['value'] = $supplier['ImsSupplier']['tin'];
