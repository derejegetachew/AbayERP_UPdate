...
var store_parent_grnItems = new Ext.data.Store({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_grn_items', 'action' => 'list_data', $parent_id)); ?>'	})
});
...
function AddParentGrnItem() {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_grn_items', 'action' => 'add', $parent_id)); ?>',
...
function EditParentGrnItem(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_grn_items', 'action' => 'edit')); ?>/'+id+'/<?php echo $parent_id; ?>',
		success: function(response, opts) {
			var parent_grnItem_data = response.responseText;
...
function ViewGrnItem(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_grn_items', 'action' => 'view')); ?>/'+id,
		success: function(response, opts) {
...
function ViewGrnItemStores(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_stores', 'action' => 'index2')); ?>/'+id,
		success: function(response, opts) {
...
function DeleteParentGrnItem(id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array('controller' => 'ims_grn_items', 'action' => 'delete')); ?>/'+id,
		success: function(response, opts) {
...
function SearchByParentGrnItemName(value){
	var conditions = '\'ImsGrnItem.ImsPurchaseOrderItem.ImsItem.name LIKE\' => \'%' + value + '%\'';
	filter_conditions = conditions;
    store_parent_grnItems.setBaseParam('query', filter_conditions);
...
var g = new Ext.grid.GridPanel({
    ...
    id: 'grnItemGrid',
	columns: [
		{header:"<?php __('Item'); ?>", dataIndex: 'purchase_order_item', sortable: true},
