		...
		?>
		var ImsDisposalItemAddForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
			...
			items: [
				...
				?>,
				{
					...
					store: new Ext.data.ArrayStore({
						...
						data: [
						<?php foreach($results as $ims_item){?>
						['<?php echo $ims_item['id']?>','<?php echo $ims_item['name']?>','<?php echo $ims_item['description']?>'],
						<?php
						}
						?>
						]
		...
		var ImsDisposalItemAddWindow = new Ext.Window({
			title: '<?php __('Add Ims Disposal Item'); ?>',
			width: 400,
			minWidth: 400,
			autoHeight: true,
			layout: 'fit',
			...
			buttonAlign:'right',
			items: ImsDisposalItemAddForm,
			tools: [{
				id: 'refresh',
				qtip: 'Reset',
				handler: function () {
					ImsDisposalItemAddForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					Ext.Msg.show({
						...
						msg: 'This form is used to insert a new Ims Disposal Item.',
				...
				handler: function () {
					if(ImsDisposalItemAddWindow.collapsed)
						ImsDisposalItemAddWindow.expand(true);
					else
						ImsDisposalItemAddWindow.collapse(true);
				}
			}],
			buttons: [  {
				text: '<?php __('Save'); ?>',
				handler: function(btn){
					ImsDisposalItemAddForm.getForm().submit({
				...
				handler: function(btn){
					ImsDisposalItemAddForm.getForm().submit({
						...
						success: function(f,a){
							...
							ImsDisposalItemAddWindow.close();
<?php if(isset($parent_id)){ ?>
							RefreshParentImsDisposalItemData();
<?php } else { ?>
							RefreshImsDisposalItemData();
