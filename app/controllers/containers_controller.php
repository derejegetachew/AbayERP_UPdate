...
class ContainersController extends AppController {
    ...
    $checkName=$container['Container']['name'];
    if($checkName=='ESS' || $checkName=='Other Systems' || $checkName== 'Employee Performance Mgt'|| $checkName=='Supervisor'){
     continue;
     }
     //}
			...
			foreach ($links as $link) {
				if ($this->__permitted($link['Link']['controller'],
								$link['Link']['action'])) {
									...
									if($link['Link']['controller']=='jobs' && $link['Link']['action']=='index3'){//for vacancy post
										$this->loadModel('Job');
										$conditions['Job.end_date >='] = date('Y-m-d');
										$conditions['Job.start_date <='] = date('Y-m-d');
										$conditions['Job.status'] = 'Open';		
										$result=$this->Job->find('count', array('conditions' => $conditions));
										$link['Link']['name']=$link['Link']['name'].' ('.$result.')';
									}
